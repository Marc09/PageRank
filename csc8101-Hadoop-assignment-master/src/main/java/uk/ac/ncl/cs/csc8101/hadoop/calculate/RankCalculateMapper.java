...
public class RankCalculateMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
    	//convert the value which contains "[page]  [initialPagerank]   outLinkA,outLinkB,outLinkC..."
    	
    	String links = value.toString();
    	String[] temp = links.split("\t");
    	String page = temp[0];
    	String initialPageRank = temp[1];
    	String stored = "";
    	//representing <Bar, "! Foo">
    	String message = "!";
    	String[] outlink = null;
    	if (temp.length == 3){
    		outlink = temp[2].split(",");
        	message = message + temp[2];
    		for (int i = 0; i < outlink.length; i++){
        		stored = initialPageRank + "\t" + outlink.length;
        		context.write(new Text(outlink[i]), new Text(stored));
        	}
    		context.write(new Text(page), new Text(message));
    	} else {
